function [solData, solYALMIP, objective] = solveProblemMRA(params)
    % Problem parameters
    ...
    R = params.R;
    M = params.M;
    eps_single = params.eps_single;

    % Optimization parameters
    paramOpt = calcOptimizationParameters(params);
    x_adv_nom = paramOpt.x_adv_nom;
    mean_advX = paramOpt.mean_advX;
    var_advX = paramOpt.var_advX;
    r1 = paramOpt.r1;
    r2 = paramOpt.r2;
    x_0 = paramOpt.x_0;
    x_r = paramOpt.x_r;
    ...
    for i = 1:HORIZON
        ...
        % Adversary
        advX_geq = norminv(1-eps_single) * sqrt(var_advX(i + 1)+r2(i + 1)) + mean_advX(i+1) + r1(i+1);
        advX_leq = -norminv(1-eps_single) * sqrt(var_advX(i + 1)+r2(i + 1)) + mean_advX(i+1) - r1(i+1);
        constraints = [constraints, x_var(1, i + 1) + CAR_LENGTH / 2 <= advX_leq - CAR_LENGTH / 2 + M * t_adv(1, i)];
        constraints = [constraints, x_var(1, i + 1) - CAR_LENGTH / 2 >= advX_geq + CAR_LENGTH / 2 - M * t_adv(2, i)];
